...
if(isset($_POST['register_button'])){
	$fname=mysqli_real_escape_string($Connection,$_POST['reg_fname']);
	$lname=str_replace(' ', '', $fname);//remove spaces..
	$fname=ucfirst(strtolower($fname));
	$_SESSION['reg_fname']=$fname;//store first name into session variable
 
	
	$lname=mysqli_real_escape_string($Connection,$_POST['reg_lname']);
	$lname=str_replace(' ', '', $lname);//remove spaces..
	$lname=ucfirst(strtolower($lname));
    $_SESSION['reg_lname']=$lname;

    $collname=mysqli_real_escape_string($Connection,$_POST['Collname']);
	$collname=str_replace(' ', '', $collname);//remove spaces..
	$collname=ucfirst(strtolower($collname));
    $_SESSION['Collname']=$collname;

  /*  $info=mysqli_real_escape_string($Connection,$_POST['Info']);
	$info=str_replace(' ', '', $info);//remove spaces..
	$info=ucfirst(strtolower($info));
    $_SESSION['Info']=$info;*/

    $department=mysqli_real_escape_string($Connection,$_POST['Department']);
	$department=str_replace(' ', '', $department);//remove spaces..
	...
	$email2=mysqli_real_escape_string($Connection,$_POST['reg_email2']);
	$email2=str_replace(' ', '', $email2);//remove spaces..
	$email2=ucfirst(strtolower($email2));
	$_SESSION['reg_email2']=$email2;

	$uniqueColId=mysqli_real_escape_string($Connection,$_POST['UniqueColId']);
	$uniqueColId=str_replace(' ', '', $uniqueColId);//remove spaces..
	$uniqueColId=ucfirst(strtolower($uniqueColId));
	$_SESSION['UniqueColId']=$uniqueColId;//store first name into session variable

    $subjectItrst=mysqli_real_escape_string($Connection,$_POST['SubjectIntrst']);
	$subjectItrst=str_replace(' ', '', $subjectItrst);//remove spaces..
	$subjectItrst=ucfirst(strtolower($subjectItrst));
	$_SESSION['SubjectIntrst']=$email2;
    $date="y-m-d"; 
	...
	$password2=mysqli_real_escape_string($Connection,$_POST['reg_password2']);
	...
	if ($email==$email2) {
		if (filter_var($email,FILTER_VALIDATE_EMAIL)) {
			...
			$check=mysqli_query($Connection,"SELECT email FROM studentregister where email='$email'");
    ...
    if (strlen($password>40||strlen($password<5))) {
		...
		$username=strtolower("Guru"." ".$fname."_".$lname);
		$check_user_query=mysqli_query($Connection,"SELECT username from social where username='$username'");
		$i=0;
		while(mysqli_num_rows($check_user_query)!=0){
			$i++;
			$username=$username."_".$i;
			$check_user_query=mysqli_query($Connection,"SELECT username from teacherregister where username='$username'");
   		}//end of while
	//	$rand=rand(1,2);
		//if($rand==1){
	   ...
	   // else if($rand==2{
		...
		$query="INSERT INTO teacherregister (firstname,lastname,username,collegename,skills,email,password,date,profile,uniqueColId,subjectintrst, info,department,user_closed) VALUES ('$fname','$lname','$username','$collname','not mentioned','$email','$password','$datetime','$profile','$uniqueColId','$SubjectIntrst','not mentioned','$department','NO')";
